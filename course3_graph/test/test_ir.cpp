...
TEST(test_ir, pnnx_graph_all) {
    ...
    for (const auto& operator_ : operators) {
        LOG(INFO) << "op name: " << operator_->name
                  << " type: " << operator_->type;
        LOG(INFO) << "attribute:";
...
TEST(test_ir, pnnx_graph_all_homework) {
    ...
    for (const auto& operator_ : operators) {
        if (operator_->name == "linear") {
            ...
            ASSERT_EQ(
                (dynamic_cast<RuntimeParameterBool*>(parameter_bool)->value),
                true);
            /////////////////////////////////
            ...
            ASSERT_EQ((dynamic_cast<RuntimeParameterInt*>(parameter_in_features)
                           ->value),
                      32);

            /////////////////////////////////
            ASSERT_EQ(params.count("out_features"), 1);
            RuntimeParameter* parameter_out_features =
                params.at("out_features");
            ASSERT_NE(parameter_out_features, nullptr);
            ASSERT_EQ(
                (dynamic_cast<RuntimeParameterInt*>(parameter_out_features)
                     ->value),
                128);
        }
    }
